 ...
 */
package org.openmrs.module.keaddonexample.wrapper;
...
public class Enrollment extends AbstractObjectWrapper<PatientProgram> {
	...
	public Obs firstObs(Concept concept) {
		...
		for (Obs obs : obss) {
			if (obs.getObsDatetime().compareTo(target.getDateEnrolled()) >= 0
			        && (target.getDateCompleted() == null || obs.getObsDatetime().compareTo(target.getDateCompleted()) < 0)) {
				return obs;
			}
		}
		return null;
	...
	public Encounter lastEncounter(EncounterType type) {
		List<Encounter> encounters = Context.getEncounterService().getEncounters(target.getPatient(), null,
		    target.getDateEnrolled(), target.getDateCompleted(), null, Collections.singleton(type), null, null, null, false);
	...
	public Encounter encounterByForm(EncounterType encounterType, Form form) {
		...
		List<Encounter> encounters = encounterService.getEncounters(target.getPatient(), null, target.getDateEnrolled(),
		    target.getDateCompleted(), forms, Collections.singleton(encounterType), null, null, null, false);
		return encounters.size() > 0 ? encounters.get(encounters.size() - 1) : null;
	}
}
